...
public class CollectHealth : IBaseState
{
    ...
    public void GetNearestHealthItem()
    {
        ...
        if (healthItem.gameObject.GetComponent<HealthItem>().active == false)
        {
            healthItem = GameManager2.Instance._lhealthItemList.OrderBy(x => Vector3.Distance(x.transform.position, transform.position)).Skip(1).First();
        }
    }
